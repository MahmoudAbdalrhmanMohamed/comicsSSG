  ...
  <div class="flex items-center gap-2">
    <div class="flex items-center gap-1">
      <div
        v-for="star in stars"
        :key="star"
        class="relative w-4 h-4 md:w-[22px] md:h-[22px]"
      >
        <!-- Empty Star -->
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="absolute inset-0 text-gray-300 w-4 h-4 md:w-[22px] md:h-[22px]"
          viewBox="0 0 24 24"
        >
          <path
            fill="currentColor"
    ...
    <p class="text-bf font-semibold text-lg hidden md:block">({{ value }})</p>
  </div>
</template>
