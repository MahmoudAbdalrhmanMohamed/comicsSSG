<template>
  <div class="flex gap-4">
    <button
      ...
      :key="star"
      :class="[
        'flex items-center justify-center rounded-full transition duration-300',
...
const rating = ref(props.selected);
const hoverRating = ref(0);

const setRating = (star) => {
  rating.value = star;
  emit("stars", rating.value);
};

// Watch for changes in the `selected` prop and update the `rating` state
watch(
  () => props.selected,
  (newValue) => {
