<template>
  <section class="mb-8 overflow-hidden">
    ...
    <div v-if="data?.data?.data_messages?.length > 0" class="px-2 md:px-12">
      <transition-group
        tag="div"
        name="page"
        appear
      ...
      >
        <NuxtLink
          ...
          class="flex md:w-[302px] flex-col relative w-full h-[407px] md:h-[502px] md:hover:rounded-md group md:hover:shadow-combined-hover cursor-pointer transition-all duration-300"
          v-for="item in data?.data?.data_messages || []"
          :key="item"
        >
          <div
          ...
          <div class="block">
            <v-lazy-image
              ...
              :src="item.image"
              :alt="`${item.full_name || $t('latestAlts.comicTitle')} ${$t('latestAlts.coverArt')} by ${item.writer}`"
              class="transition-all w-full object-fill md:group-hover:h-[345px] md:w-[302px] h-[280px] md:h-[400px] duration-300 md:group-hover:rounded-b-lg flex-shrink"
            />
          </div>

          <p
            class="hidden transition-all truncate md:w-[302px] md:block md:group-hover:hidden duration-300 font-semibold text-white md:text-2xl pt-4 pb-2"
          >
            {{ item.full_name }}
          </p>
          <p
          ...
          >
            ...
            <PublicStars
              :count="Number(item.rateCount) || 0"
              :value="Number(item.rate) || 0"
            />
            <p class="text-sm font-medium">
              {{ item.genres.map((genere) => genere.name).join(", ") }}
            </p>
            <div
...
// Adjust the range of visible pages based on device type
...
const { data } = await useFetch(
  computed(() => `${url}/home/latest-comics?page=${currentPage.value}`),
  {
    headers: {
...
const toggleFavComic = async (id) => {
  try {
    if (useCookie("authToken").value) {
      ...
      if (response?.status) {
        if (data.value) {
          const indexVal = data.value.data.data_messages.findIndex(
            (item) => item.id === id
          );
          if (indexVal !== -1) {
            data.value.data.data_messages[indexVal].checkFavorite =
              !data.value.data.data_messages[indexVal].checkFavorite;
          }
        }
